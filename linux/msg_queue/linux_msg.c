...
int PushMsgQueue(const int queue_id, void *p_buff, const int buff_size) {
	...
	if (NULL == p_buff || MSG_BUFF_MAX <= buff_size) {
		...
		snd_ret = msgsnd(queue_id, &msg, sizeof(msg), IPC_NOWAIT);
		if (-1 == snd_ret) {
			printf("msgsnd() error! errno=%s\n", (char *)strerror(errno));
			return -1;
		}
		else
			return buff_size;
...
int PopMsgQueue(const int queue_id, void *p_buff, const int buff_size) {
	...
	if (NULL == p_buff) {
		...
		msg.m_type = MSG_TYPE;
		rcv_ret = msgrcv(queue_id, &msg, sizeof(msg), 0, IPC_NOWAIT); // 先进先出
		if (-1 == rcv_ret) {
			printf("msgrcv() error! errno=%s\n", (char *)strerror(errno));
			return -1;
		}
		else {
			if (sizeof(msg) < rcv_ret || buff_size > rcv_ret - sizeof(msg.m_type)) {
